...
def export_onnx(model, im, file, opset, train,export2, dynamic, simplify, prefix=colorstr('ONNX:')):
    # YOLOv5 ONNX export
    try:
        ...
        if train or export2:
            out_names=["l80","l40","l20","seg"]
        else:
            out_names=['output','seg']
...
def export_engine(model, im, file, train,export2, half, simplify, workspace=4, verbose=False, prefix=colorstr('TensorRT:')):
    try:
        check_requirements(('tensorrt',))
        import tensorrt as trt
        
        opset = (12, 13)[trt.__version__[0] == '8']  # test on TensorRT 7.x and 8.x
        export_onnx(model, im, file, opset, train,export2, False, simplify)
...
def run(data=ROOT / 'data/coco128.yaml',  # 'dataset.yaml path'
    ...
    if 'onnx' in include:
        export_onnx(model, im, file, opset, train, export2, dynamic, simplify)
    if 'engine' in include:
        export_engine(model, im, file, train, export2, half, simplify, workspace, verbose)
...
def parse_opt():
    ...
    parser.add_argument('--export2', action='store_true', help='custom export')
